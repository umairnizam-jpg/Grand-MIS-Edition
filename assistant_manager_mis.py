    ...
    if st.session_state.get("authentication_status"):
        ...
        with chat_container:
            ...
            if st.session_state.chart_data is not None:
                ...
                t1, t2 = st.tabs(["🎯 Achievements", "📈 Growth Trends"])
                with t1:
                    ...
                    c3.plotly_chart(px.pie(f_df, values='Actual Revenue', names='Months', title="Rev Share").update_layout(height=250, showlegend=False))
                    c4.plotly_chart(px.bar(f_df, x='Months', y='Actual Revenue', title="Monthly Rev").update_layout(height=250))
                with t2:
                    tc1, tc2 = st.columns(2)
                    tc1.plotly_chart(px.line(f_df, x='Months', y=['Actual Revenue', 'Target revenue'], title="Rev vs Target").update_layout(height=250))
                    tc2.plotly_chart(px.area(f_df, x='Months', y='Actual Footfall', title="FF Flow").update_layout(height=250))
        ...
        if prompt: 
            ...
            user_query = "Voice command received (Analyzing...)"
        ...
        if user_query:
            ...
            # DATA FILTERING (Error Prevention)
            filtered_df = df_live.copy() # FIX: Initialized here to avoid UnboundLocalError
            ...
            if not df_live.empty:
                ...
                if not filtered_df.empty:
                    act_r = filtered_df["Actual Revenue"].sum()
                    tar_r = filtered_df["Target revenue"].sum()
                    act_f = filtered_df["Actual Footfall"].sum()
                    tar_f = filtered_df["Target Footfall"].sum()

                    rev_ach = (act_r / tar_r * 100) if tar_r > 0 else 0
                    ff_ach = (act_f / tar_f * 100) if tar_f > 0 else 0

                    st.session_state.chart_data = {'df': filtered_df, 'rev_ach': rev_ach, 'ff_ach': ff_ach}
                    st.session_state.messages.append({"content": f"📊 Analysis complete. Revenue Achieved: {rev_ach:.1f}%. Visuals updated.", "is_user": False})
                    st.rerun()
                else:
                    st.session_state.messages.append({"content": "No data found for this period.", "is_user": False})
                    st.rerun()
