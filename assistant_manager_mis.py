...
# --- ADVANCED ISLAMIC EVENT FORECASTING ENGINE ---
...
def main():
    ...
    if st.session_state.get("authentication_status"):
        ...
        st.sidebar.info("Scope: 2017 – 2030 (AI Ready)")
        ...
        with input_col:
            prompt = st.chat_input("Ask about Revenue, Footfall, Forecasts...")
        ...
        if prompt:
            ...
            if any(f in query_lower for f in ['forecast', 'predict', 'btao', 'prediction']):
                ...
                if found_m and found_y:
                    ...
                    eid_note = " 🌙 **(Islamic Event Detected)**" if (y_val in [2026,2027,2028,2029,2030] and m_idx in [1,2,3,4,5,6]) else ""
                    ans = (f"### 🔮 AI Prediction for {found_m.capitalize()} {y_val}\n"
                           f"* Projected Revenue: **Rs. {p_rev:,.0f}**{eid_note}\n"
                           f"* Projected Footfall: **{p_ff:,.0f} Pax**\n"
                           f"--- \nNote: Adjusted for festive seasons.")
                    st.session_state.messages.append({"content": ans, "is_user": False})
                    st.rerun()

            # --- ORIGINAL COMPARISON ENGINE ---
            ...
            if "vs" in query_lower:
                ...
                if len(parts) == 2:
                    ...
                    v1, l1 = get_period_df(parts[0]); v2, l2 = get_period_df(parts[1])
                    if not v1.empty and not v2.empty:
                        ...
                        variance_report = f"\n\n**Growth:** Revenue {r_perc:+.1f}%, Footfall {f_perc:+.1f}%"
            ...
            if not temp_df.empty:
                ...
                st.session_state.messages.append({"content": f"### 📊 Result\n* Rev: **Rs. {res['Actual Revenue']:,.0f}**\n* FF: **{res['Actual Footfall']:,.0f}**{variance_report}", "is_user": False})
                st.rerun()

        # --- VISUALS TAB (100% ORIGINAL WITH 6 CHARTS) ---
        if st.session_state.last_filtered_df is not None:
            df_plot = st.session_state.last_filtered_df
            tab1, tab2 = st.tabs(["📉 Visual Insights", "🔮 Forecast Center"])
            with tab1:
                if st.session_state.comparison_data:
                    c = st.session_state.comparison_data
                    st.plotly_chart(go.Figure([go.Bar(name='Revenue', x=c['labels'], y=c['revenue'], marker_color='#00CC96'), 
                                               go.Bar(name='Footfall', x=c['labels'], y=c['footfall'], marker_color='#636EFA')]).update_layout(barmode='group', template="plotly_dark"), use_container_width=True)
                ...
                if chart_option.startswith("1"):
                    ...
                    st.plotly_chart(go.Figure(go.Indicator(mode="gauge+number", value=ach, gauge={'bar':{'color':"#00CC96"}})).update_layout(height=300, template="plotly_dark"), use_container_width=True)
            ...
            with tab2:
                # Prediction Graphs (Original + AI Extension)
                st.subheader("2017-2030 Performance Trend")
                fig_f = go.Figure()
                fig_f.add_trace(go.Scatter(x=df_live['Date_Obj'], y=df_live['Actual Revenue'], name="Historical"))
